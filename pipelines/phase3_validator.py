...
class TestDoltImporterImportOrder:
    ...
    def test_import_all_skips_missing_curated_files(self, tmp_path):
        ...
        assert all(v["status"] == "skipped" for v in report["tables"].values())
...
class TestSampleGenerator:
    ...
    def test_public_samples_creates_files(self, tmp_path):
        from pipelines.publish.sample_generator import (
            SampleGenerator,
        )
...
class TestManifestBuilder:
    def _make_export_files(self, tmp_path):
        ...
        SampleGenerator._write_parquet.__func__(
            None, pd.DataFrame([{"a": 1}]), tmp_path / "test.parquet"
        ) if False else None
