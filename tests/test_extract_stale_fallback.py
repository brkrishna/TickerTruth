...
def test_fetch_uses_stale_cache_when_all_live_methods_fail(
    ...
    monkeypatch.setattr(
        extractor, "_fetch_corp_actions_playwright", lambda *a, **kw: None
    )
...
def test_staleness_fresh_file_is_silent(extractor, tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING):
        extractor._check_bhavcopy_staleness(
            date(2026, 6, 1)
        )  # 2 days gap — under threshold
    assert "stale" not in caplog.text.lower()
...
def test_consolidate_passes_run_date_to_staleness_check(
    extractor, tmp_path, monkeypatch, caplog
):
