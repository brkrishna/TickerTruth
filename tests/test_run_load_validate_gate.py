...
def test_commit_skipped_when_validate_failed(monkeypatch):
    ...
    with patch("pipelines.publish.dolt_importer.DoltImporter", return_value=importer):
        ok = run_load(RUN_DATE, dry_run=False, no_dolt_commit=False, validate_passed=False)

    assert ok is True, "load should still report success — import ran, commit intentionally skipped"
...
def test_commit_runs_when_validate_passed(monkeypatch):
    ...
    importer = _mock_importer()
    with patch("pipelines.publish.dolt_importer.DoltImporter", return_value=importer):
        ok = run_load(RUN_DATE, dry_run=False, no_dolt_commit=False, validate_passed=True)
...
def test_commit_skipped_by_dry_run_regardless_of_validate(monkeypatch):
    ...
    importer = _mock_importer()
    with patch("pipelines.publish.dolt_importer.DoltImporter", return_value=importer):
        ok = run_load(RUN_DATE, dry_run=True, no_dolt_commit=False, validate_passed=True)
...
def test_commit_skipped_by_no_dolt_commit_flag(monkeypatch):
    ...
    importer = _mock_importer()
    with patch("pipelines.publish.dolt_importer.DoltImporter", return_value=importer):
        ok = run_load(RUN_DATE, dry_run=False, no_dolt_commit=True, validate_passed=True)
...
def test_main_passes_validate_failure_to_load(monkeypatch):
    ...
    def fake_run_load(run_date, dry_run, no_dolt_commit, validate_passed=True):
