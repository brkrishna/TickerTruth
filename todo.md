...
- [x] **BUG-8 (correctness, small effort, MEDIUM impact) — FIXED 2026-08-14** — `SymbolLinker.cross_reference_with_actions` (`pipelines/lineage/linker.py`) mutated the caller's `actions` DataFrame in place (`actions["_action_date"] = ...` then `drop(..., inplace=True)`), violating the explicit "never mutate input DataFrames" rule in `pipelines/lineage/CLAUDE.md`. Fixed by computing parsed dates into a local `action_dates` Series instead of assigning a column onto `actions` — no column is ever added to the caller's DataFrame. Covered by `tests/test_lineage_linker.py::test_cross_reference_does_not_mutate_actions_input`.
...
- [x] **TEST-1 (HIGH priority — mandated by CLAUDE.md) — lineage/adjustments FIXED 2026-08-14** — Added `tests/test_lineage_rules.py`, `tests/test_lineage_linker.py`, `tests/test_adjustments_calculator.py`, `tests/test_adjustments_adjuster.py` (84 tests; two split from the single `test_adjustments_factors.py` CLAUDE.md names, matching this repo's actual one-file-per-source-module convention rather than the doc's aspirational single filename). Surfaced and fixed two real bugs in the process: a hash-seed-dependent non-determinism bug in `SymbolLinker.link_across_periods` (see `tasks.md` for the full writeup) and BUG-8 above. `pipelines/adjustments/`'s test cases for RIGHTS, FACE_VALUE_CHANGE, duplicate-event dedup, and missing-date UNRESOLVED flags were **not** added — those depend on action types/columns (`confidence_flag`, `duplicate_group_id`) that `calculator.py`/`adjuster.py` don't implement yet; see `tasks.md` for the spec-vs-implementation gap. **Still open:** `test_normalize_normalizer.py` (core NSE `RawToCanonicalMapper`/`quality.py`) and `test_extract_extractor.py` (general extractor coverage beyond the stale-fallback path).
