...
- [x] **TEST-1 (HIGH priority — mandated by CLAUDE.md) — lineage/adjustments/normalize FIXED 2026-08-14** — Added `tests/test_lineage_rules.py`, `tests/test_lineage_linker.py`, `tests/test_adjustments_calculator.py`, `tests/test_adjustments_adjuster.py` (84 tests; two split from the single `test_adjustments_factors.py` CLAUDE.md name, matching this repo's actual one-file-per-source-module convention rather than the doc's aspirational single filename). Surfaced and fixed two real bugs: a hash-seed-dependent non-determinism bug in `SymbolLinker.link_across_periods` and BUG-8 above (see `tasks.md` for the full writeup). `pipelines/adjustments/`'s test cases for RIGHTS, FACE_VALUE_CHANGE, duplicate-event dedup, and missing-date UNRESOLVED flags were **not** added — those depend on action types/columns (`confidence_flag`, `duplicate_group_id`) that `calculator.py`/`adjuster.py` don't implement yet; see `tasks.md` for the spec-vs-implementation gap.
